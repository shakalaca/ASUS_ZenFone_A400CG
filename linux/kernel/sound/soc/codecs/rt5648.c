...
static struct rt5648_init_reg init_list[] = {
	...
	{ RT5648_HPO_MIXER	, 0x5000 },/* HPOVOL -> HPOLMIX */
#endif
...
void rt5648_i2s2_func_switch(struct snd_soc_codec *codec, bool enable)
{
        snd_soc_update_bits(codec, RT5648_GPIO_CTRL1,
                RT5648_I2S2_SEL, (!enable) << RT5648_I2S2_SEL_SFT);
}
EXPORT_SYMBOL(rt5648_i2s2_func_switch);
...
static const struct snd_kcontrol_new rt5648_snd_controls[] = {
	...
	/* I2S2 function select */
        SOC_SINGLE("I2S2 Func Switch", RT5648_GPIO_CTRL1,
                                RT5648_I2S2_SEL_SFT, 1, 1),
        /* TDM */
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		snd_soc_write(w->codec, RT5648_ASRC_1, 0xffff);
		snd_soc_write(w->codec, RT5648_ASRC_2, 0x1111);
                snd_soc_update_bits(w->codec, RT5648_SIDETONE_CTRL,
						RT5648_ST_SEL_MASK, tmp);
		snd_soc_update_bits(w->codec, RT5648_ADDA_CLK1, 0xf, 0x5);
		break;
	case SND_SOC_DAPM_PRE_PMD:
                snd_soc_update_bits(w->codec, RT5648_ADDA_CLK1, 0xf, 0);
		snd_soc_write(w->codec, RT5648_ASRC_1, 0x0);
		snd_soc_write(w->codec, RT5648_ASRC_2, 0x0);
		break;
	default:
		return 0;
	}
