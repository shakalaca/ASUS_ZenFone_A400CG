...
static int __init specific_gpio_setting_init()
{
	int PROJ_ID = Read_PROJ_ID();
	int HW_ID = Read_HW_ID();
	int PCB_ID = Read_PCB_ID();
	//ZC400CF ME171CG are no longer as 3 bits format, but 6 bits.
	int project_id =  (PROJ_ID>56) ? PROJ_ID >> 3 : PROJ_ID;
...
//		printk("[test] project_id=%#x PROJ_ID=%#x\n",project_id,PROJ_ID);
	}
...
module_init(specific_gpio_setting_init);
