...
*/
	...
	{"imx219", RAW_CAMERA, ATOMISP_CAMERA_PORT_PRIMARY},
        {"ov5670", SOC_CAMERA, ATOMISP_CAMERA_PORT_PRIMARY},
					...
					struct devs_id *dev)
{
	...
	if (!strcmp(i2c_info.type, "mt9m114")){
        ...
        if (!strcmp(i2c_info.type, "ov5670")){
                i2c_info.addr = 0x10;
                printk("name:%s, addr=0x%x", i2c_info.type, i2c_info.addr);
        }

        if (!strcmp(i2c_info.type, "gc0310")){
                i2c_info.addr = 0x21; //fake value for adding node
                printk("name:%s, addr=0x%x", i2c_info.type, i2c_info.addr);
        }

	if (!strcmp(i2c_info.type, "ar0543_raw")){
#ifdef CONFIG_ZC400CG  //ZC400CG hack
		i2c_info.addr = 0x10;
		strncpy(i2c_info.type, "ov5670", SFI_NAME_LEN);
		printk("name:%s, addr=0x%x", i2c_info.type, i2c_info.addr);
#else
		i2c_info.addr = 0x36;
		printk("name:%s, addr=0x%x", i2c_info.type, i2c_info.addr);
...
#ifdef CONFIG_ZC400CG  //ZC400CG hack
                i2c_info.addr = 0x21;
                strncpy(i2c_info.type, "gc0310", SFI_NAME_LEN);
                printk("name:%s, addr=0x%x", i2c_info.type, i2c_info.addr);
#else
                i2c_info.addr = 0x21; //fake value for adding node
                printk("name:%s, addr=0x%x", i2c_info.type, i2c_info.addr);
	...
	if (!strcmp(i2c_info.type, "imx219")){
		i2c_info.addr = 0x10;
		printk("name:%s, addr=0x%x", i2c_info.type, i2c_info.addr);
	}

	if (!strcmp(i2c_info.type, "t4k35")){
		i2c_info.addr = 0x36;
		printk("name:%s, addr=0x%x", i2c_info.type, i2c_info.addr);
	}
	//Peter--
