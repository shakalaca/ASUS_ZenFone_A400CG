...
/* This value overwrite the default value of ULPI_VS1SET
 * which is used to optimize eye diagram */
static u8 ulpi_vs1set;
...
void penwell_update_transceiver(void)
{
	...
	if (!pnw->queue_stop) {
		queue_delayed_work(pnw->qwork, &pnw->work, 0.5*HZ);
...
static void penwell_otg_eye_diagram_optimize(void)
{
    ...
    if (ulpi_vs1set & 0x7f > 0) {
        value = ulpi_vs1set;
        pr_info("ULPI_VS1SET overwrite with %02x\n", value);
    }
...
#ifdef CONFIG_EEPROM_PADSTATION
	if (station_mode && !p72_cable_event){
...
#ifdef CONFIG_DEBUG_FS
static ssize_t ulpi_vs1set_write(struct file *filp,
                                 ...
                                 loff_t *ppos)
{
    ...
    if (sscanf(buf, "%02hx", &tmp) != 1)
		return -EINVAL;

    ulpi_vs1set = (u8)(tmp & 0x7f);
    pr_info("ULPI_VS1SET = %02x\n", ulpi_vs1set);

    return count;
}

static ssize_t ulpi_vs1set_read(struct file *filp, char __user *ubuf,
				size_t cnt, loff_t *ppos)
{
    char buf[32];

    snprintf(buf, sizeof(buf), "ULPI_VS1SET = %02x\n", ulpi_vs1set);

    return simple_read_from_buffer(ubuf, cnt, ppos,
                                   buf, strlen(buf));
}

static int ulpi_vs1set_open(struct inode *inode, struct file *filp)
{
    filp->private_data = inode->i_private;
	return 0;
}

static int ulpi_vs1set_release(struct inode *inode, struct file *file)
{
	return 0;
}
static const struct file_operations ulpi_vs1set_fops = {
	.open	 = ulpi_vs1set_open,
	.read	 = ulpi_vs1set_read,
	.release = ulpi_vs1set_release,
    .write   = ulpi_vs1set_write,
	.llseek	 = default_llseek,
};

static struct dentry *penwell_otg_dbg_root;
static struct dentry *penwell_otg_dbg_ulpi_vs1set;

static int penwel_otg_debugfs_init(struct penwell_otg *pnw)
{
	penwell_otg_dbg_root = debugfs_create_dir("penwell_otg", NULL);

	if (!penwell_otg_dbg_root || IS_ERR(penwell_otg_dbg_root))
		return -ENODEV;

	penwell_otg_dbg_ulpi_vs1set = debugfs_create_file("ulpi_vs1set",
                                                      S_IRUGO | S_IWUSR,
                                                      penwell_otg_dbg_root, pnw,
                                                      &ulpi_vs1set_fops);
	if (!penwell_otg_dbg_ulpi_vs1set) {
		debugfs_remove(penwell_otg_dbg_root);
		penwell_otg_dbg_root = NULL;
		return -ENODEV;
	}

	return 0;
}
static void penwel_otg_debugfs_cleanup(void)
{
	debugfs_remove(penwell_otg_dbg_ulpi_vs1set);
	debugfs_remove(penwell_otg_dbg_root);
}
#endif
