...
static int langwell_udc_reset(struct langwell_udc *dev)
{
	...
	if (build_version == 1 && entry_mode ==1){
		//Eric add for configure usb in full speed mode.
		//build_version = 1 = eng. entry_mode = 1 = MOS.
		if (switch_speed == 1)
			devlc |= LPM_PFSC;
		else
			devlc &= ~LPM_PFSC;
...
{
    return udc_init_completed;
}
EXPORT_SYMBOL(langwell_udc_init_completed);
