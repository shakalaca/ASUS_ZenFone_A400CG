...
static int ax88772b_reset(struct usbnet *dev)
{
	...
	for (i = 0; i < (ETH_ALEN >> 1); i++) {
		if ((ret = ax8817x_read_cmd (dev, AX_CMD_READ_EEPROM, 0x04 + i,
					     0, 2, (buf + i * 2))) < 0)
			      ...
			      struct ethtool_eeprom *eeprom, u8 *data)
{
	...
	PhyReg = ax8817x_mdio_read_le (dev->net,
				dev->mii.phy_id, 4);
...
ax88178_media_check (struct usbnet *dev, struct ax88178_data *ax178dataptr)
{
	...
	do {
		...
		_stat1000 = ax8817x_mdio_read_le (dev->net,
