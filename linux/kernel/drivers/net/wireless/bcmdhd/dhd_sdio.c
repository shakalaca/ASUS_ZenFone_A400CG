...
//Fix driver bug: get BCM4334x SR support
...
dhdsdio_suspend(void *context)
{
	...
	printk ("dhdsdio_suspend: dhd_os_check_wakelock=%d, bus->dhd->up=%d\n", ret, bus->dhd->up);
	ret = dhd_os_check_wakelock(bus->dhd);
	if ((!ret) && (bus->dhd->up)) {

		//Force set bus->sleeping = TRUE for CHIP without SR support
		if (!SLPAUTO_ENAB(bus)){
			printk("dhdsdio_suspend: CHIP without SR support, set bus->sleeping=TRUE when suspend\n");
			bus->sleeping = TRUE;
		}

		if (wait_event_timeout(bus->bus_sleep, bus->sleeping, wait_time) == 0) {
			if (!bus->sleeping) {
				printk("dhdsdio_suspend:  bus is not sleeping, return 1\n");
				return 1;
			}
		}
	}
...
{
	printf("dhdsdio_resume!!!!!!!!!\n");
	dhd_bus_t *bus = (dhd_bus_t*)context;

	//set bus->sleeping = FALSE for CHIP without SR support
	if (!SLPAUTO_ENAB(bus)){
		printf("dhdsdio_resume:  CHIP without SR support, set bus->sleeping = FALSE when resume\n");
		bus->sleeping = FALSE;
	}
