	...
	if (is_wps_conn(sme)) {
		if (sme->privacy)
			err = wldev_iovar_setint_bsscfg(dev, "wsec", 4, bssidx);
				...
				err = wldev_iovar_setint_bsscfg(dev, "wsec", gval, bssidx);
			}
#endif
		else
	...
	struct cfg80211_connect_params *sme)
{
	...
	if (chan) {
		...
		wl->channel = 0;
#ifdef BCMWAPI_WPI
	WL_DBG(("wpa_versions (%d)\n", sme->crypto.wpa_versions));
	if (sme->crypto.wpa_versions & NL80211_WAPI_VERSION_1) {
		WL_DBG(("2. WAPI set wapi ie\n"));
		err = wl_set_set_wapi_ie(dev, sme);
		if (unlikely(err))
			return err;
	} else
		WL_DBG(("2. Don't set wapi ie\n"));
#endif
...
#ifdef BCMWAPI_WPI
	if (sme->crypto.wpa_versions & NL80211_WAPI_VERSION_1)
		WL_DBG(("4. WAPI don't set auth type\n"));
	else {
		WL_DBG(("4. Set auth type\n"));
#endif
