...
//CY+ send disassoc to FW when scan error several times to break FW busy
static int cnt = 0;
//CY-
...
//CY+ send disassoc to FW when scan error several times to break FW busy
			...
			cnt++;
			WL_ERR(("##### wl_run_escan(): error (%d), cnt=%d\n", err, cnt));
			...
			if (cnt >= 4) {
				...
				WL_ERR(("##### wl_run_escan(): Send disassoc to break the busy dev=%p\n", dev));
				cnt = 0;
			}
		}
	}else {
...
#ifdef WL_SCHED_SCAN
#if defined(WIFI_CHIP_43362)
#define PNO_TIME		600
#elif defined(WIFI_CHIP_4343S)
#define PNO_TIME		500
#else
#define PNO_TIME		500
#endif
...
static s32 wl_setup_wiphy(struct wireless_dev *wdev, struct device *sdiofunc_dev, void *context)
{
	...
	//Set wowlan.flags = WIPHY_WOWLAN_ANY
	printf("wl_setup_wiphy: force set wowlan.flags = WIPHY_WOWLAN_ANY\n");
	wdev->wiphy->wowlan.flags = WIPHY_WOWLAN_ANY;
