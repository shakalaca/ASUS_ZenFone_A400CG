...
static bool g_force_charge = false;
...
static int balance_judge()
{
    ...
    if (pad_ac_present) {
        ...
        if (IsBalanceMode == 1) {
            ...
            if (asus_pad_device->ec_suspend_status == STATUS_ACTIVE) {
                Init_Microp_Vbus(true);
                ...
                if (phone_batt_rsoc >= 90) {
                    ...
                    if (get_sw_charging_toggle() == false)
                        Init_Microp_Vbus(false);
                }
            }
#if 0
...
static void DoBalance()
{
    ...
    if ((phone_batt_rsoc >= 90 || (phone_batt_rsoc - pad_rsoc) >= 0) && phone_batt_rsoc >= 20) {
        pr_err("PBATT> %s: * request CUT OFF pad power supply * !\n", __func__);
        Init_Microp_Vbus(false);
        smb345_charging_toggle(BALANCE, false);
    }
    else if (phone_batt_rsoc <= 80 && phone_batt_rsoc <= pad_rsoc * 90 / 100) {
        if ( isSuspend == STATUS_ACTIVE ) {
            Init_Microp_Vbus(true);
            if (phone_batt_rsoc >= 20) {
                ...
                pr_warn("PBATT> %s: F:%d%%, P:%d%% !\n",
                    __func__, phone_batt_rsoc, pad_rsoc);
            }
        }
        else {
        ...
        /* when the above 2 conditions are not statified,
           still need to control phone battery charging.
        */
        if ( isSuspend == STATUS_ACTIVE) {
            if (phone_batt_rsoc >= 20) {
                pr_warn("PBATT> %s: * request RECOVERY power supply"
            ...
            else if (((phone_batt_rsoc >= 90 || (phone_batt_rsoc - pad_rsoc) >= 0)) && (!g_force_charge)) {
        		pr_err("PBATT> %s: * request CUT OFF pad power supply * !!\n", __func__);
        		Init_Microp_Vbus(false);
        		smb345_charging_toggle(BALANCE, false);
        	}
...
static int read_pad_bat_current(char *buf, char **start, off_t off, int count, int *eof, void *data)
{
    int len = 0;
...
static int read_pad_bat_usoc(char *buf, char **start, off_t off, int count, int *eof, void *data)
{
    int len = 0;
...
static int read_pad_temp(char *buf, char **start, off_t off, int count, int *eof, void *data)
{
    int len = 0;
...
static int create_proc_pad_node(void)
{
    ...
    ent = create_proc_read_entry("Pad_Bat_Voltage", 0744, NULL, read_pad_bat_voltage, NULL);
    if(!ent)
    {
        pr_info("PBATT> %s: failed to create /proc/Pad_Bat_Voltage\n", __func__);
        rtn = rtn - 1;
    }
    ent = create_proc_read_entry("Pad_Bat_Current", 0744, NULL, read_pad_bat_current, NULL);
    if(!ent)
    {
        pr_info("PBATT> %s: failed to create /proc/Pad_Bat_Current\n", __func__);
        rtn = rtn - 1;
    }
    ent = create_proc_read_entry("Pad_Bat_USOC", 0744, NULL, read_pad_bat_usoc, NULL);
    if(!ent)
    {
        pr_info("PBATT> %s: failed to create /proc/Pad_Bat_USOC\n", __func__);
        rtn = rtn - 1;
    }
    ent = create_proc_read_entry("Pad_Temp", 0744, NULL, read_pad_temp, NULL);
    if(!ent)
    {
...
#endif	///< end of ASUS_EC_POWER_PROC_DEV

#ifdef CONFIG_HAS_EARLYSUSPEND
static void asus_pad_batt_early_suspend(struct early_suspend *h)
{
    asus_pad_device->ec_suspend_status = STATUS_EARLY_SUSPEND;
    printk("enter %s\n", __func__);
    return 0;

}

static void asus_pad_batt_late_resume(struct early_suspend *h)
{
    printk("%s ++\r\n",__FUNCTION__);
    asus_pad_device->ec_suspend_status = STATUS_ACTIVE;
    return 0;
}

#else
#define asus_pad_batt_early_suspend NULL
...
static int asus_ec_power_probe(struct platform_device *pdev)
{
    ...
    asus_pad_device->IsBalanceMode = 0;
#else
...
#ifdef CONFIG_HAS_EARLYSUSPEND
    asus_pad_device->es.level = EARLY_SUSPEND_LEVEL_DISABLE_FB + 1;
    asus_pad_device->es.suspend = asus_pad_batt_early_suspend;
    asus_pad_device->es.resume = asus_pad_batt_late_resume;
    register_early_suspend(&asus_pad_device->es);
#endif
...
static int asus_pad_batt_suspend(struct device *dev)
{
    ...
    mutex_lock(&ec_info_lock);
    asus_pad_device->ec_suspend_status = STATUS_SUSPEND;
    mutex_unlock(&ec_info_lock);
