...
static irqreturn_t sdhci_pci_sd_cd(int irq, void *dev_id)
{
	...
	mmc_detect_change(host->mmc,
                      (gpio_status == 0) ? msecs_to_jiffies(700) : msecs_to_jiffies(500));
	return IRQ_HANDLED;
}
...
#else
#define sdhci_pci_set_dev_power NULL
#endif
...
static ssize_t SD_status(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    if (pdev->device != PCI_DEVICE_ID_INTEL_CLV_SDIO0)
        return 0;
				     ...
				     const struct pci_device_id *ent)
{
    ...
    if (pdev->device == PCI_DEVICE_ID_INTEL_CLV_SDIO0 ||
        pdev->device == PCI_DEVICE_ID_INTEL_CLV_EMMC0) {
