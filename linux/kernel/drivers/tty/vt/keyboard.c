...
static void kbd_keycode(unsigned int keycode, int down, int hw_raw)
{
	...
	if(0>shift_final || shift_final>=MAX_NR_KEYMAPS){//Illegal index access
		printk("[Key_map]Error detected!! keycode=%d shift_state=%#x kbd->slockstate=%#x kbd->lockstate=%#x\n",
			keycode,shift_state,kbd->slockstate,kbd->lockstate);
		printk("[UTS_KEY_MAP]%s:%s(parent:%s): tgid=%d\n", __func__, current->comm, current->parent->comm, current->tgid);
		int i,j;
		unsigned short *map;
		...
		shift_state = kbd->slockstate = kbd->lockstate = 0;
		return;
	}
	...
	if (keycode < NR_KEYS){
		printk("[Key_map]key_map=%#x keycode=%d shift_state=%#x kbd->slockstate=%#x kbd->lockstate=%#x",
			key_map,keycode,shift_state,kbd->slockstate,kbd->lockstate);
		if( key_map > 0xc0000000 ){
			keysym = key_map[keycode];
			printk(" keysym=%#x\n",keysym);
		}
		else{	//Null point excption detected!!
			printk(" Error detected!!! Dump all key map.\n");
			printk("[UTS_KEY_MAP]%s:%s(parent:%s): tgid=%d\n", __func__, current->comm, current->parent->comm, current->tgid);
			int i,j;
			unsigned short *map;
			for(i=0;i<MAX_NR_KEYMAPS;i++){
						...
						int console)
{
	...
	case KDSKBENT:
		...
		if (key_map == NULL) {
			...
			kfree(new_map);
		printk("[Key_map]Warning!! key_maps may be rewrited by user command.\n");
		printk("[Key_map]%s:%s(parent:%s): tgid=%d\n", __func__, current->comm, current->parent->comm, current->tgid);
		printk("[Key_map]key_map=%x key_maps=%x s=%d\n",key_map,key_maps,s);
		dump_stack();
		ov = U(key_map[i]);
		if (v == ov)
			goto out;
