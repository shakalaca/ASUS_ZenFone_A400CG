...
//	invensense solution for TT288428 ++
/*
	u8 *datatmp;
	u8 a_buf[6];    
	short a_raw[THREE_AXIS];
*/
//	invensense solution for TT288428 --
	conf = &st->chip_config;
...
/*
	{
		if ((conf->accl_fifo_enable) && !(conf->gyro_fifo_enable) && !(conf->compass_fifo_enable))
		{
			result = inv_i2c_read(st, st->reg.raw_accl, BYTES_PER_SENSOR, a_buf);
			if (result)
				printk(KERN_INFO "i2c error\n");
			a_raw[0] = be16_to_cpup((__be16 *)(&a_buf[0]));
			a_raw[0] *= st->chip_info.multi;
			a_raw[1] = be16_to_cpup((__be16 *)(&a_buf[2]));
			a_raw[1] *= st->chip_info.multi;
			a_raw[2] = be16_to_cpup((__be16 *)(&a_buf[4]));
			a_raw[2] *= st->chip_info.multi;
			if ((a_raw[0]!=a[0]) && (a_raw[1]!=a[1]) && (a_raw[2]!=a[2])) {        
				printk(KERN_INFO "error, reg_accel=%x\n", st->reg.raw_accl);
				printk(KERN_INFO "mpu6500 : a     = %x %x %x\n",a[0],a[1],a[2]);
				printk(KERN_INFO "mpu6500 : a_raw = %x %x %x\n",a_raw[0],a_raw[1],a_raw[2]);
			}
			//a[0] = a_raw[0];
			//a[1] = a_raw[1];
			//a[2] = a_raw[2];
...
irqreturn_t inv_read_fifo(int irq, void *dev_id)
{
	...
	if (bytes_per_datum != 0) {
		...
		if( (fifo_count%bytes_per_datum)!=0 ){
