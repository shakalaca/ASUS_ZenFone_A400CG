...
struct i2c_client *main_client; //ASUS_BSP+++
...
//ASUS_BSP++
uint32_t gsensor_info_for_camera_imu(void)
{
	uint8_t buffer[6];
	int err=0;
	int rawdata_x=0,rawdata_y=0,rawdata_z=0;
	...
	buffer[0] = REG_RAW_ACCEL;
	err = mpu6500_i2c_rxdata(main_client, &(buffer[0]), 6);
	if (err < 0){
		printk("get_mpu6500_gsensor_rawdata fail\n");
		return err;
	}
	...
	data_x = rawdata_x*98/16384;
	data_y = rawdata_y*98/16384;
	data_z = rawdata_z*98/16384;
	...
	//printk("Accel Rawdata : (%d), (%d), (%d)\n",rawdata_x, rawdata_y, rawdata_z);
	all = (((uint8_t)data_x<<16)|((uint8_t)data_y<<8)|((uint8_t)data_z))&0xffffff;
	return all;
}
EXPORT_SYMBOL_GPL(gsensor_info_for_camera_imu);
//ASUS_BSP--
