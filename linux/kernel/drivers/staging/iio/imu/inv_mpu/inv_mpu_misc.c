		     ...
		     u32 len, u8 const *data)
{
	...
	int res;
	printk("mpu6500: mpu_memory_write +\n");
	...
	if (res != 1) {
		...
		printk("mpu6500: mpu_memory_write success\n");
		return 0;
	}
}
		    ...
		    u32 len, u8 *data)
{
	...
	printk("mpu6500: mpu_memory_read +\n");
	if (!data || !st)
		return -EINVAL;
		...
		int *gyro_result, int *accl_result)
{
	...
	printk("mpu6500: inv_do_test abs packet_count(%d)\n",packet_count);			
	if (abs(packet_count - DEF_GYRO_PACKET_THRESH) > DEF_GYRO_THRESH){
		printk("mpu6500: inv_do_test packet_count out of range\n");		
		return -EAGAIN;
	}
