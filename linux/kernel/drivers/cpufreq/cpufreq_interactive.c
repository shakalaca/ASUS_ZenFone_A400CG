...
static void cpufreq_interactive_timer(unsigned long data)
{
	...
	if (new_freq > tunables->platform_max_freq)
		new_freq = tunables->platform_max_freq;
...
static ssize_t show_platform_max_freq(struct cpufreq_interactive_tunables
                *tunables, char *buf)
{
        return sprintf(buf, "%lu\n", tunables->platform_max_freq);
}

static ssize_t store_platform_max_freq(struct cpufreq_interactive_tunables
                                *tunables, const char *buf, size_t count)
{
        int ret;
        unsigned long val;
        ...
        if (ret < 0)
                return ret;
        tunables->platform_max_freq = val;
        return count;
}
...
show_store_gov_pol_sys(platform_max_freq);
...
gov_sys_pol_attr_rw(platform_max_freq);
...
static struct attribute *interactive_attributes_gov_sys[] = {
	...
	&platform_max_freq_gov_sys.attr,
		...
		unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		if (!tunables->platform_max_freq)
			tunables->platform_max_freq = policy->max;
