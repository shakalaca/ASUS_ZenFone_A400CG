...
#include <linux/i2c/gt927_firmware_1040.h> // modify by leo
#endif
...
#define UPDATE_FILE_PATH_3          "/system/etc/firmware/_goodix_update_.bin" // add by leo

#define CONFIG_FILE_PATH_1          "/data/_goodix_config_.cfg"
#define CONFIG_FILE_PATH_2          "/sdcard/_goodix_config_.cfg"
#define CONFIG_FILE_PATH_3          "/system/etc/firmware/_goodix_config_.cfg" // add by leo
...
extern int gt927_fw_check;
extern int gt927_update_result;
extern int gt927_check_result;
...
static int update_touch_progress(int update_progress); // add by leo for touch update progress 
...
static u8 gup_enter_update_judge(st_fw_head *fw_head)
{
    ...
    if ( !memcmp(fw_head->hw_info, update_msg.ic_fw_msg.hw_info, sizeof(update_msg.ic_fw_msg.hw_info)))
    {
		...
		GTP_INFO("FILE FW version > IC FW version.");  	
	     	gt927_check_result=1;
	     }else if(fw_head->vid == update_msg.ic_fw_msg.vid){
	       GTP_INFO("FILE FW version = IC FW version."); 
	     	gt927_check_result=2;
	     }else{
	       GTP_INFO("FILE FW version < IC FW version."); 
	     	gt927_check_result=3;
	     }
	     // add by leo --
			
            //The third condition
            if ((fw_head->vid > update_msg.ic_fw_msg.vid)||gt927_ForceFWUpdate)
            {
...
static u8 gup_check_update_file(struct i2c_client *client, st_fw_head* fw_head, u8* path)
{
    ...
    else
    {
        ...
        for (i = 0; i < GUP_SEARCH_FILE_TIMES; i++)
        {
            ...
            {
            	if(gt927_ForceFWUpdate||gt927_fw_check){
		      		update_msg.file = filp_open(UPDATE_FILE_PATH_3, O_RDONLY, 0); // add by leo
            	}else{
            ...
            if(!(got_file_flag&0xF0))
            {
		  		...
		  		if(gt927_ForceFWUpdate||gt927_fw_check){
					update_msg.cfg_file = filp_open(CONFIG_FILE_PATH_3, O_RDONLY, 0); // add by leo
		  		}else{
	     ...
	     if(((gt927_ForceFWUpdate==1)||(gt927_fw_check==1))&&(i>3))break;	 // add by leo
        }
...
// add by leo for touch update progress ++
static int update_touch_progress(int update_progress){
	...
	if(ts->progress < update_progress || update_progress == 0){
		ts->progress = update_progress;
		...
		if(!IS_ERR_OR_NULL(filePtr)) {
			...
			printk("////////////////////		%s:[%d]: %s not found\n", __func__, __LINE__,Progress_file_path);
			return 1;
		} else {
			printk("////////////////////		%s:[%d]: %s open error\n", __func__, __LINE__,Progress_file_path);
			return 1;
		}
	}
}
...
// add by leo ++
int gt927_FW_update_check(void){
    ...
    ret = gup_check_update_file(i2c_connect_client, &fw_head, NULL);
    if(FAIL == ret)
    {
        GTP_ERROR("[update_proc]check update file fail.");
        goto gt927_check_fail;
    }
...
gt927_check_fail:
    ...
    if(update_msg.file && !IS_ERR(update_msg.file))
    {
     ...
     printk("////////////////////		%s:[%d]: NO Need enter update \n", __func__, __LINE__);	
    return 0;
}
// add by leo --
...
s32 gup_update_proc(void *dir)
{
    ...
    gtp_irq_enable(ts);
    filp_close(update_msg.file, NULL);
    gt927_update_result=0; // add by leo
    gt927_ForceFWUpdate=0; // add by leo
...
update_fail:
    ...
    show_len = 200;
    total_len = 100;
    gt927_update_result=-1; // add by leo	
    gt927_ForceFWUpdate=0; // add by leo
    update_touch_progress(100);
    return FAIL;
}
...
u8 gup_init_update_proc(struct goodix_ts_data *ts)
{
    ...
    got_file_flag=0; // add by leo
    gt927_update_result=1;
