...
//=============================================================================================================
	...
	static struct i2c_client	*touch_i2c												= NULL;					// for I2C transfer
...
//=============================================================================================================
	...
	static int himax_touch_sysfs_init(void)
	{
        ...
        if (ret)
        {
            printk(KERN_ERR "[TP]TOUCH_ERR: sysfs_create_file dev_attr_touch_status failed\n");
            return ret;
        }
        ...
        if (ret)
        {
            printk(KERN_ERR "[TP]TOUCH_ERR: sysfs_create_file dev_attr_tp_check_running failed\n");
            return ret;
        }
        ...
        if (ret)
        {
            printk(KERN_ERR "[TP]TOUCH_ERR: sysfs_create_file dev_attr_touch_irq failed\n");
            return ret;
        }
        ...
        if (ret)
        {
            printk(KERN_ERR "[TP]TOUCH_ERR: sysfs_create_file dev_attr_tp_fw_version failed\n");
            return ret;
        }
        ...
        if (ret)
        {
            printk(KERN_ERR "[TP]TOUCH_ERR: sysfs_create_file dev_attr_debug_log failed\n");
            return ret;
        }
		//add by Josh -------
...
//=============================================================================================================
	...
	//----[ i2c ]---------------------------------------------------------------------------------------------start
		static int himax_ts_probe(struct i2c_client *client,const struct i2c_device_id *id)
		{
			...
			ts->client 				= client;
			ts->init_success 	= 0;
			ts->tp_status 	= 0; // add by josh for check poweron
			...
			if( gpio_request(HIMAX_INT_GPIO, "HimaxTouch-irq") != 0 )
			{
...
{
	printk(KERN_ERR "[Himax_ME372CGCD] %s: power_status: %d \n", __func__, status);
	return 0;
