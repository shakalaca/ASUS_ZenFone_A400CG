...
struct i2c_client	*himax_i2c										= NULL;		// for I2C transfer
...
unsigned char c36[]  =  { 0x40,0x01, 0x5A, 0x77, 0x02, 0xF0, 0x13, 0x00, 0x00, 0x56, 0x10, 0x14, 0x18, 0x06, 0x10, 0x0C, 0x0F, 0x0F, 0x0F, 0x52, 0x34, 
						0x80, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};//start:0x00 ,size 31

unsigned char c37[]  =  { 0x40, 0xA5, 0x00, 0x80, 0x82, 0x85, 0x00, 0x35, 0x25, 0x0F, 0x0F, 0x83, 0x3C, 0x00, 0x00
						, 0x11, 0x11, 0x00, 0x00, 0x01, 0x01, 0x00, 0x0A, 0x00, 0x00
...
unsigned char c40[]  =  { 0x40, 0x10, 0x12, 0x20, 0x32, 0x01, 0x04, 0x07, 0x09, 0xB4, 0x6E, 0x32, 0x00
						, 0x0F, 0x1C, 0xA0, 0x16, 0x00, 0x00, 0x04, 0x38, 0x07, 0x80}; //start 0x64,size 23
...
unsigned char c43_1[] = { 0x40, 0x00, 0xFF, 0x15, 0x28, 0x01, 0xFF, 0x16, 0x29, 0x02, 0xFF, 0x1B, 0x2A, 0x03, 0xFF, 0x1C, 0xFF, 0x04, 0xFF, 0x1D, 
0xFF, 0x05, 0x0F, 0x1E, 0xFF, 0x06, 0x10, 0x1F, 0xFF, 0x07, 0x11, 0x20}; //start 0x9E,size 32
unsigned char c43_2[] = { 0x40, 0xFF, 0x08, 0x12, 0x21, 0xFF, 0x09, 0x13, 0x22, 0xFF, 0x0A, 0x14, 0x23, 0xFF, 0x0B, 0x17, 0x24, 0xFF, 0x0C, 0x18,
0x25, 0xFF, 0x0D, 0x19, 0x26, 0xFF, 0x0E, 0x1A, 0x27, 0xFF}; //start 0xBD,size 29

unsigned char c44_1[] = { 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}; //start 0xDA,size 32
...
static int hx8528_chip_self_test(uint8_t *data);
#endif 
...
static uint8_t rFE96_setting[8] = { 0xB4, 0x64, 0x3F, 0x3F, 0x3C, 0x00, 0x3C, 0x00}; //add by josh for init self_test FE96
...
//=============================================================================================================
	//----[ normal function]--------------------------------------------------------------------------------start
		...
		//----[CONFIG_HAS_EARLYSUSPEND]-----------------------------------------------------------------------start
			...
			static void himax_ts_early_suspend(struct early_suspend *h)
			{
				...
				himax_ts_suspend(private_ts->client, PMSG_SUSPEND);
			}
			...
			static void himax_ts_late_resume(struct early_suspend *h)
			{
				...
				printk(KERN_INFO "[Himax] %s\n", __func__);
				//ts = container_of(h, struct himax_ts_data, early_suspend);
				himax_ts_resume(private_ts->client);
			}
			#endif
		//----[CONFIG_HAS_EARLYSUSPEND]-------------------------------------------------------------------------end	
	//----[ normal function]----------------------------------------------------------------------------------end
	...
	//----[ i2c read/write function]--------------------------------------------------------------------------end
		...
		{   
			...
			if( i2c_himax_master_write(himax_i2c, &c1[0],sizeof(c1),DEFAULT_RETRY_CNT) < 0)
			{
				goto HimaxErr;
			}
			
			if( i2c_himax_master_write(himax_i2c, &c2[0],sizeof(c2),DEFAULT_RETRY_CNT) < 0)
			{
				goto HimaxErr;
			}
			
			if( i2c_himax_master_write(himax_i2c, &c3[0],sizeof(c3),DEFAULT_RETRY_CNT) < 0)
			{
				goto HimaxErr;
			}
			
			if( i2c_himax_master_write(himax_i2c, &c4[0],sizeof(c4),DEFAULT_RETRY_CNT) < 0)
			{
				goto HimaxErr;
			}
			
			if( i2c_himax_master_write(himax_i2c, &c5[0],sizeof(c5),DEFAULT_RETRY_CNT) < 0)
			{
				goto HimaxErr;
			}
			
			if( i2c_himax_master_write(himax_i2c, &c6[0],sizeof(c6),DEFAULT_RETRY_CNT) < 0)
			{
				goto HimaxErr;
			}
			
			if( i2c_himax_master_write(himax_i2c, &c7[0],sizeof(c7),DEFAULT_RETRY_CNT) < 0)
			{
				goto HimaxErr;
			}
			
			if( i2c_himax_master_write(himax_i2c, &c8[0],sizeof(c8),DEFAULT_RETRY_CNT) < 0)
			{
				goto HimaxErr;
			}
			
			if( i2c_himax_master_write(himax_i2c, &c9[0],sizeof(c9),DEFAULT_RETRY_CNT) < 0)
			{
				goto HimaxErr;
			}
			
			if( i2c_himax_master_write(himax_i2c, &c10[0],sizeof(c10),DEFAULT_RETRY_CNT) < 0)
			{
				goto HimaxErr;
			}
			
			if( i2c_himax_master_write(himax_i2c, &c11[0],sizeof(c11),DEFAULT_RETRY_CNT) < 0)
			{
				goto HimaxErr;
			}
			
			if( i2c_himax_master_write(himax_i2c, &c12[0],sizeof(c12),DEFAULT_RETRY_CNT) < 0)
			{
				goto HimaxErr;
			}
			
			if( i2c_himax_master_write(himax_i2c, &c13[0],sizeof(c13),DEFAULT_RETRY_CNT) < 0)
			{
				goto HimaxErr;
			}
			
			if( i2c_himax_master_write(himax_i2c, &c14[0],sizeof(c14),DEFAULT_RETRY_CNT) < 0)
			{
				goto HimaxErr;
			}
			
			if( i2c_himax_master_write(himax_i2c, &c15[0],sizeof(c15),DEFAULT_RETRY_CNT) < 0)
			{
				goto HimaxErr;
			}
			
			if( i2c_himax_master_write(himax_i2c, &c16[0],sizeof(c16),DEFAULT_RETRY_CNT) < 0)
			{
				goto HimaxErr;
			}
			
			if( i2c_himax_master_write(himax_i2c, &c17[0],sizeof(c17),DEFAULT_RETRY_CNT) < 0)
			{
				goto HimaxErr;
			}
			
			if( i2c_himax_master_write(himax_i2c, &c18[0],sizeof(c18),DEFAULT_RETRY_CNT) < 0)
			{
				goto HimaxErr;
			}
			
			if( i2c_himax_master_write(himax_i2c, &c19[0],sizeof(c19),DEFAULT_RETRY_CNT) < 0)
			{
				goto HimaxErr;
			}
			
			if( i2c_himax_master_write(himax_i2c, &c20[0],sizeof(c20),DEFAULT_RETRY_CNT) < 0)
			{
				goto HimaxErr;
			}
			
			if( i2c_himax_master_write(himax_i2c, &c21[0],sizeof(c21),DEFAULT_RETRY_CNT) < 0)
			{
				goto HimaxErr;
			}
			
			if( i2c_himax_master_write(himax_i2c, &c22[0],sizeof(c22),DEFAULT_RETRY_CNT) < 0)
			{
				goto HimaxErr;
			}
			
			if( i2c_himax_master_write(himax_i2c, &c23[0],sizeof(c23),DEFAULT_RETRY_CNT) < 0)
			{	
				goto HimaxErr;
			}
			
			if( i2c_himax_master_write(himax_i2c, &c24[0],sizeof(c24),DEFAULT_RETRY_CNT) < 0)
			{
				goto HimaxErr;
			}
			
			if( i2c_himax_master_write(himax_i2c, &c25[0],sizeof(c25),DEFAULT_RETRY_CNT) < 0)
			{
				goto HimaxErr;
			}
			
			if( i2c_himax_master_write(himax_i2c, &c26[0],sizeof(c26),DEFAULT_RETRY_CNT) < 0)
			{
				goto HimaxErr;
			}
			
			if( i2c_himax_master_write(himax_i2c, &c27[0],sizeof(c27),DEFAULT_RETRY_CNT) < 0)
			{
				goto HimaxErr;
			}
			
			if( i2c_himax_master_write(himax_i2c, &c28[0],sizeof(c28),DEFAULT_RETRY_CNT) < 0)
			{
				goto HimaxErr;
			}
			
			if( i2c_himax_master_write(himax_i2c, &c29[0],sizeof(c29),DEFAULT_RETRY_CNT) < 0)
			{
				goto HimaxErr;
			}
			
			if( i2c_himax_master_write(himax_i2c, &c30[0],sizeof(c30),DEFAULT_RETRY_CNT) < 0)
			{
				goto HimaxErr;
			}
			
			if( i2c_himax_master_write(himax_i2c, &c31[0],sizeof(c31),DEFAULT_RETRY_CNT) < 0)
			{
				goto HimaxErr;
			}
			
			if( i2c_himax_master_write(himax_i2c, &c32[0],sizeof(c32),DEFAULT_RETRY_CNT) < 0)
			{
				goto HimaxErr;
			}
			
			if( i2c_himax_master_write(himax_i2c, &c33[0],sizeof(c33),DEFAULT_RETRY_CNT) < 0)
			{
				goto HimaxErr;
			}
			
			if( i2c_himax_master_write(himax_i2c, &c34[0],sizeof(c34),DEFAULT_RETRY_CNT) < 0)
			{
				goto HimaxErr;
			}
			
			if( i2c_himax_master_write(himax_i2c, &c35[0],sizeof(c35),DEFAULT_RETRY_CNT) < 0)
			{
				goto HimaxErr;
			}
			...
			data[0] = 0xAB;
			data[1] = 0x00;
			if((i2c_himax_master_write(himax_i2c, &data[0],2,3))<0)
			{
				goto HimaxErr;
			}
			...
			if((i2c_himax_master_write(himax_i2c, &data[0],3,3))<0)
			{
				goto HimaxErr;
			}			
					
			if((i2c_himax_master_write(himax_i2c, &c36[0],sizeof(c36),3))<0)
			{	
				goto HimaxErr;
			}
			...
			if((i2c_himax_master_write(himax_i2c, &c37[0],sizeof(c37),3))<0)
			{	
				goto HimaxErr;
			}
			...
			if((i2c_himax_master_write(himax_i2c, &c38[0],sizeof(c38),3))<0)
			{	
				goto HimaxErr;
			}
		  ...
		  //------------------------------------------------------------------config bank PART c39 START
			...
			if((i2c_himax_master_write(himax_i2c, &c39[0],sizeof(c39),3))<0)
			{	
				goto HimaxErr;
			}
		  ...
		  //------------------------------------------------------------------config bank PART c40 START
			...
			if((i2c_himax_master_write(himax_i2c, &c40[0],sizeof(c40),3))<0)
			{	
				goto HimaxErr;
			}
			...
			if((i2c_himax_master_write(himax_i2c, &c41[0],sizeof(c41),3))<0)
			{	
				goto HimaxErr;
			}
			...
			if((i2c_himax_master_write(himax_i2c, &c42[0],sizeof(c42),3))<0)
			{	
				goto HimaxErr;
			}
			...
			if((i2c_himax_master_write(himax_i2c, &c43_1[0],sizeof(c43_1),3))<0)
			{	
				goto HimaxErr;
			}
			...
			if((i2c_himax_master_write(himax_i2c, &c43_2[0],sizeof(c43_2),3))<0)
			{	
				goto HimaxErr;
			}
			...
			if((i2c_himax_master_write(himax_i2c, &c44_1[0],sizeof(c44_1),3))<0)
			{	
				goto HimaxErr;
			}
			...
			if((i2c_himax_master_write(himax_i2c, &c44_2[0],sizeof(c44_2),3))<0)
			{	
				goto HimaxErr;
			}
			...
			if((i2c_himax_master_write(himax_i2c, &c45[0],sizeof(c45),3))<0)
			{	
				goto HimaxErr;
			}
			...
			int ret = i2c_himax_read(himax_i2c, 0xAB, buf0, 2, DEFAULT_RETRY_CNT);
		  if(ret < 0)
			{
	...
	//----[ register flow function]-------------------------------------------------------------------------start
		static int himax_ts_poweron(struct himax_ts_data *ts_modify)
		{
			...
			else if (IC_TYPE == HX_85XX_E_SERIES_PWON)
			{
				...
				buf0[11] = 0x00;
				buf0[12] = 0x00; 
				ret = i2c_himax_master_write(ts_modify->client, buf0, 13, DEFAULT_RETRY_CNT);
				if(ret < 0) 
				{
					printk(KERN_ERR "i2c_master_send failed addr = 0x%x\n",ts_modify->client->addr);
					goto send_i2c_msg_fail;
		...
		static int himax_ManualMode(int enter)
		{
			uint8_t cmd[2];
			cmd[0] = enter;
			if( i2c_himax_write(himax_i2c, 0x42 ,&cmd[0], 1, DEFAULT_RETRY_CNT) < 0)
			{
		...
		static int himax_FlashMode(int enter)
		{
			uint8_t cmd[2];
			cmd[0] = enter;
			if( i2c_himax_write(himax_i2c, 0x43 ,&cmd[0], 1, DEFAULT_RETRY_CNT) < 0)
			{
				printk(KERN_ERR "[TP] %s: i2c access fail!\n", __func__);
				return 0;
			}
		...
		static int himax_lock_flash(void)
		{
			...
			if( i2c_himax_write(himax_i2c, 0x43 ,&cmd[0], 3, DEFAULT_RETRY_CNT) < 0)
			{
				printk(KERN_ERR "[TP] %s: i2c access fail!\n", __func__);
				return 0;
			}
			...
			if( i2c_himax_write(himax_i2c, 0x44 ,&cmd[0], 3, DEFAULT_RETRY_CNT) < 0)
			{
				printk(KERN_ERR "[TP] %s: i2c access fail!\n", __func__);
				return 0;
			}
			...
			if( i2c_himax_write(himax_i2c, 0x45 ,&cmd[0], 4, DEFAULT_RETRY_CNT) < 0)
			{
				printk(KERN_ERR "[TP] %s: i2c access fail!\n", __func__);
				return 0;
			}
			
			if( i2c_himax_write_command(himax_i2c, 0x4A, DEFAULT_RETRY_CNT) < 0)
			{
				printk(KERN_ERR "[TP] %s: i2c access fail!\n", __func__);
				return 0;
			}
		...
		static uint8_t himax_calculateChecksum(char *ImageBuffer, int fullLength)//, int address, int RST)
		{
			...
			if(IC_CHECKSUM == HX_TP_BIN_CHECKSUM_SW)
			{
				...
				for (i = 0; i < FileLength; i++) 
				{
					...
					if( i2c_himax_write_command(himax_i2c, 0x46, DEFAULT_RETRY_CNT) < 0)
					{
						printk(KERN_ERR "[TP] %s: i2c access fail!\n", __func__);
						return 0;
					}
					
					if( i2c_himax_read(himax_i2c, 0x59, cmd, 4, DEFAULT_RETRY_CNT) < 0)
					{
						printk(KERN_ERR "[TP] %s: i2c access fail!\n", __func__);
						return -1;
					}
			...
			else if(IC_CHECKSUM == HX_TP_BIN_CHECKSUM_HW)
			{
				...
				for (i = 0; i < FileLength; i++) 
				{
					...
					else 
					{
						...
						cmd[0] = 0x01;
						if( i2c_himax_write(himax_i2c, 0xE5 ,&cmd[0], 1, DEFAULT_RETRY_CNT) < 0)	
						{
							printk(KERN_ERR "[TP] %s: i2c access fail!\n", __func__);
							return 0;
						}
						...
						//Get HW Checksum. 
						if( i2c_himax_read(himax_i2c, 0xAD, cmd, 4, DEFAULT_RETRY_CNT) < 0)
						{
							printk(KERN_ERR "[TP] %s: i2c access fail!\n", __func__);
							return -1;
						}
			...
			else if(IC_CHECKSUM == HX_TP_BIN_CHECKSUM_CRC)
			{
				...
				if(IC_TYPE == HX_85XX_E_SERIES_PWON)
				{
					...
					if( i2c_himax_read(himax_i2c, 0xED, cmd, 5, DEFAULT_RETRY_CNT) < 0)
					{
						printk(KERN_ERR "[TP] %s: i2c access fail!\n", __func__);
						return -1;
					}
					...
					if( i2c_himax_write(himax_i2c, 0xED ,&cmd[0], 5, DEFAULT_RETRY_CNT) < 0)
					{
						printk(KERN_ERR "[TP] %s: i2c access fail!\n", __func__);
						return 0;
					}
					...
					//Select CRC Mode
					cmd[0] = 0x05;
					if( i2c_himax_write(himax_i2c, 0xD2 ,&cmd[0], 1, DEFAULT_RETRY_CNT) < 0)
					{
						printk(KERN_ERR "[TP] %s: i2c access fail!\n", __func__);
						return 0;
					}
					...
					//Enable CRC Function
					cmd[0] = 0x01;
					if( i2c_himax_write(himax_i2c, 0x53 ,&cmd[0], 1, DEFAULT_RETRY_CNT) < 0)
					{
						printk(KERN_ERR "[TP] %s: i2c access fail!\n", __func__);
						return 0;
					}
				...
				else
				{
					...
					//Set Flash Clock Rate
					if( i2c_himax_read(himax_i2c, 0x7F, cmd, 5, DEFAULT_RETRY_CNT) < 0)
					{
						printk(KERN_ERR "[TP] %s: i2c access fail!\n", __func__);
						return -1;
					}
					...
					if( i2c_himax_write(himax_i2c, 0x7F ,&cmd[0], 5, DEFAULT_RETRY_CNT) < 0)
					{
						printk(KERN_ERR "[TP] %s: i2c access fail!\n", __func__);
						return 0;
					}
					...
					cmd[1] = 0x00;
					cmd[2] = 0x00;
					if( i2c_himax_write(himax_i2c, 0xD2 ,&cmd[0], 3, DEFAULT_RETRY_CNT) < 0)
					{
						printk(KERN_ERR "[TP] %s: i2c access fail!\n", __func__);
						return 0;
					} 
	...
	//----[HX_IREF_MODIFY]------------------------------------------------------------------------------------end
      ...
      u16 local_addr		   = addr_start % 4;   		
			...
			if( i2c_himax_write_command(himax_i2c, 0x81, DEFAULT_RETRY_CNT) < 0)
			{
				printk(KERN_ERR "[TP]TOUCH_ERR: %s i2c write 81 fail.\n",__func__);
				return 0;
			}
			msleep(120);
			if( i2c_himax_write_command(himax_i2c, 0x82, DEFAULT_RETRY_CNT) < 0)
			{
			...
			do
			{
				...
				if( i2c_himax_write(himax_i2c, 0x46 ,&cmd[0], 0, DEFAULT_RETRY_CNT) < 0)
				{
					printk(KERN_ERR "[TP] %s: i2c access fail!\n", __func__);
					return 0;
				}
	...
	//----[firmware version read]---------------------------------------------------------------------------start
		static u8 himax_read_FW_ver(bool hw_reset)
		{
			...
			//Sleep out
			if( i2c_himax_write(himax_i2c, 0x81 ,&cmd[0], 0, DEFAULT_RETRY_CNT) < 0)	
			{
				printk(KERN_ERR "[TP] %s: i2c access fail!\n", __func__);
				return 0;
			}
	...
	void himax_touch_information(void)
	{
		...
		else if(IC_TYPE == HX_85XX_C_SERIES_PWON)
		{
			...
			i2c_himax_write_command(himax_i2c, 0x82, DEFAULT_RETRY_CNT);
			msleep(120);
			
			//XY_REVERSE //RF1
			i2c_himax_read(himax_i2c, 0xF1, data, 2, DEFAULT_RETRY_CNT);
			...
			if((data[0] & 0x04) == 0x04)
			{	
					i2c_himax_read(himax_i2c, 0xC7, data, 4, DEFAULT_RETRY_CNT);
			...
			//RX/TX/BT channel
			i2c_himax_read(himax_i2c, 0xEA, data, 3, DEFAULT_RETRY_CNT);
			HX_RX_NUM = data[0];
			HX_TX_NUM = data[1];
			HX_BT_NUM = data[2] & 0x0F;
			
			//PT num
			i2c_himax_read(himax_i2c, 0xF0, data, 3, DEFAULT_RETRY_CNT);
			HX_MAX_PT = data[0] >> 4;
			
			i2c_himax_write_command(himax_i2c, 0x83, DEFAULT_RETRY_CNT);
			msleep(120);
		}
		else if(IC_TYPE == HX_85XX_D_SERIES_PWON)
		{
  		...
  		//Update himax information from configuration
			...
			data[0] = 0xE1;
			data[1] = 0x15;
			i2c_himax_master_write(himax_i2c, &data[0],2,DEFAULT_RETRY_CNT);
			msleep(10);
  		
			data[0] = 0xD8;
			data[1] = 0x00;
			data[2] = 0x70;
			i2c_himax_master_write(himax_i2c, &data[0],3,DEFAULT_RETRY_CNT);
			msleep(10);
  		
			i2c_himax_read(himax_i2c, 0x5A, data, 12, DEFAULT_RETRY_CNT);
			...
			i2c_himax_read(himax_i2c, 0x5A, data, 4, DEFAULT_RETRY_CNT);
			...
			i2c_himax_read(himax_i2c, 0x5A, data, 10, DEFAULT_RETRY_CNT);
	...
	//----[HX_FW_UPDATE_BY_I_FILE]--------------------------------------------------------------------------start
		...
		int fts_ctpm_fw_upgrade_with_i_file(void)
		{
			...
			for (j = 0; j < 3; j++) 
			{
				...
				if( i2c_himax_write(himax_i2c, 0x4F ,&cmd[0], 0, DEFAULT_RETRY_CNT) < 0)
				{
					printk(KERN_ERR "[TP] %s: i2c access fail!\n", __func__);
					return 0;
				}
				...
				for (i = 0, prePage = 0; i < FileLength; i++) 
				{
					...
					if (prePage != cmd[1] || i == 0) 
					{
						...
						if( i2c_himax_write(himax_i2c, 0x43 ,&cmd[0], 2, DEFAULT_RETRY_CNT) < 0)
						{
							printk(KERN_ERR "[TP] %s: i2c access fail!\n", __func__);
							return 0;
						}
...
static ssize_t hx8528_chip_self_test_function(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	val = hx8528_chip_self_test(data);
	if(val == 0)
	{
...
//----[HX_RST_PIN_FUNC]-------------------------------------------------------------------------------end
	...
	err = himax_ts_poweron(private_ts); 
	if(err == 0){
...
//=============================================================================================================
	...
	//----[HX_TP_PROC_REGISTER]------------------------------------------------------------------------------start
		...
		static ssize_t himax_register_read(struct seq_file *buf, void *v)
		{
			...
			if(multi_register_command == 1)
			{
				...
				for(loop_i = 0; loop_i < 6; loop_i++)
				{
					if(multi_register[loop_i] != 0x00)
					{
						...
						{
							outData[0] = 0x15;
							i2c_himax_write(himax_i2c, 0xE1 ,&outData[0], 1, DEFAULT_RETRY_CNT);
							msleep(10);
			
							outData[0] = 0x00;
							outData[1] = multi_register[loop_i];
							i2c_himax_write(himax_i2c, 0xD8 ,&outData[0], 2, DEFAULT_RETRY_CNT);
							msleep(10);
			
							i2c_himax_read(himax_i2c, 0x5A, inData, 128, DEFAULT_RETRY_CNT);
						...
						{
							i2c_himax_read(himax_i2c, multi_register[loop_i], inData, 128, DEFAULT_RETRY_CNT);
			...
			{
				if (i2c_himax_read(himax_i2c, register_command, inData, 128, DEFAULT_RETRY_CNT) < 0) 
				{
					return ret;
				}
			}
						...
						if (messages[base] == '\n') 
						{
							if (messages[0] == 'w')
							{
								if(config_bank_reg)
								{
									...
									msleep(10);
									i2c_himax_write(himax_i2c, 0x40, &write_da[0], length, DEFAULT_RETRY_CNT);
								...
								{
									i2c_himax_write(himax_i2c, register_command, &write_da[0], length, DEFAULT_RETRY_CNT);
									printk(KERN_INFO "CMD: %x, %x, %d\n", register_command,write_da[0], length);
								}
							}
      ...
      if (len >= 80) 
      {
			...
			if (messages[0] == '1')	//IIR
			{
				if(IC_TYPE == HX_85XX_E_SERIES_PWON)
				{
					...
					i2c_himax_write(himax_i2c, command_F1h[0] ,&command_F1h[1], 1, DEFAULT_RETRY_CNT);
				}
				else
				{
					command_91h[1] = command_ec_128_raw_baseline_flag; //A:0x03 , D:0x02
					i2c_himax_write(himax_i2c, command_91h[0] ,&command_91h[1], 1, DEFAULT_RETRY_CNT);
				}
			...
			else if (messages[0] == '3')	//BANK
			{
				if ( (IC_TYPE != HX_85XX_D_SERIES_PWON)&& (IC_TYPE != HX_85XX_E_SERIES_PWON))
				{    
					i2c_himax_write(himax_i2c, command_82h[0] ,&command_82h[0], 0, DEFAULT_RETRY_CNT);
					msleep(50);
					
					i2c_himax_read(himax_i2c, command_F3h[0], receive, 1, DEFAULT_RETRY_CNT) ;
					command_F3h[1] = (receive[0] | 0x80);
					i2c_himax_write(himax_i2c, command_F3h[0] ,&command_F3h[1], 1, DEFAULT_RETRY_CNT);
					
					command_91h[1] = command_ec_128_raw_baseline_flag;
					i2c_himax_write(himax_i2c, command_91h[0] ,&command_91h[1], 1, DEFAULT_RETRY_CNT);				
					
					i2c_himax_write(himax_i2c, command_83h[0] ,&command_83h[0], 0, DEFAULT_RETRY_CNT);
					msleep(50);
				}
				else if(IC_TYPE == HX_85XX_D_SERIES_PWON)
				{
...
static ssize_t self_test_setting(const char *buf)
{
	char buf_tmp[6], length = 0;
    uint8_t veriLen = 0;
    uint8_t write_da[100];
	...
	return count;
		...
		static int himax_chip_self_test(void)
		{
			...
			else if(IC_TYPE == HX_85XX_D_SERIES_PWON || IC_TYPE == HX_85XX_E_SERIES_PWON )
			{
				//Step 0 : sensor off
...
//=============================================================================================================
	...
	//----[ interrupt ]---------------------------------------------------------------------------------------start
		...
		static int himax_ts_register_interrupt(struct i2c_client *client)
		{
			...
			{
				err = request_irq(private_ts->client->irq, himax_ts_irq_handler,IRQF_TRIGGER_FALLING, private_ts->client->name, private_ts);
			}
			else	//low level trigger
			{*/
				err = request_irq(himax_chip->client->irq, himax_ts_irq_handler,IRQF_TRIGGER_LOW, himax_chip->client->name, himax_chip);
			//}
	...
	//----[ i2c ]---------------------------------------------------------------------------------------------start
		static int himax_ts_probe(struct i2c_client *client,const struct i2c_device_id *id)
		{
			...
			himax_i2c 				= client; //global variable
			private_ts 				= himax_chip;
			...
			himax_chip->client->irq = gpio_to_irq(himax_chip->intr_gpio);
			...
			if(err == 0){
				...
				printk("[HIMAX PORTING MSG]%s S7 : Himax Power on error=%d.\n",__func__,err);
			...
			//----[CONFIG_HAS_EARLYSUSPEND]-----------------------------------------------------------------------start
				#ifdef CONFIG_HAS_EARLYSUSPEND
				himax_chip->early_suspend.level = EARLY_SUSPEND_LEVEL_DISABLE_FB + 1;
			...
			//sysfs_remove_group(&himax_chip->client->dev.kobj, &himax_chip->attrs);
		...
		{
			//struct himax_ts_data *himax_chip = i2c_get_clientdata(client);
			
			himax_touch_proc_deinit();
			himax_touch_sysfs_deinit();
			
			unregister_early_suspend(&himax_chip->early_suspend);
			free_irq(client->irq, himax_chip);
...
int himax_cable_status(int status)
{
    ...
    if(private_ts->init_success == 1)
    {
        ...
        {
            buf0[0] = 0x00;
            i2c_himax_write(himax_i2c, 0x90 ,&buf0[0], 1, DEFAULT_RETRY_CNT);
        }
