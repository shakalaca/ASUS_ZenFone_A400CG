...
static ssize_t mxt_enable_touch(struct device *dev,struct device_attribute *attr, char *buf){
	...
	if(data->suspend_state==0 && !data->tp_firmware_upgrade_proceed && system_owner != SYSTEM_WINDOWS) error = enable_touch();
	else error = 1;
...
static ssize_t mxt_disable_touch(struct device *dev,struct device_attribute *attr, char *buf){
        ...
        if(data->suspend_state==0 && !data->tp_firmware_upgrade_proceed && system_owner != SYSTEM_WINDOWS) error = disable_touch();
	else error = 1;
