...
wl_cfgp2p_disable_discovery(struct wl_priv *wl)
{
	...
	if ( ((wl)->p2p == NULL) || ((wl)->p2p->bss == NULL) || wl_to_p2p_bss_bssidx(wl, P2PAPI_BSSCFG_DEVICE) == 0) {
		CFGP2P_ERR((" do nothing, not initialized\n"));
		goto exit;
	}
...
//CY+ prevent kernel crash when wl->p2p == null
	if(!(wl->p2p)){
		CFGP2P_ERR(("wl_cfgp2p_stop_p2p_device: wl->p2p is NULL, return directly.\n"));
		return;
	}
//CY-
