...
//CY+ disable roam before GO create and restore it after GO removed
...
//CY+ disable roam before GO create and restore it after GO removed
	        	for_each_ndev(wl, iter, next) {
                                ...
                                if ((err = wldev_iovar_setint(iter->ndev, "bus:credall", 0)) != BCME_OK)
                                        WL_ERR(("failed to set credall\n"));
                        }
//CY-
...
//CY+ fix WEP-OPEN connection problem
#ifdef CHIPNUM
	uint chipnum = dhd_get_chipnum(dev);
#endif
//CY-
	...
	case NL80211_AUTHTYPE_AUTOMATIC:
		val = WL_AUTH_OPEN_SHARED_BCM4334X;

//CY+ fix WEP-OPEN connection problem
//WL_AUTH_OPEN_SHARED is 2 in BCM43362 WIFI FW, 3 in BCM43340 WIFI FW
#ifdef CHIPNUM
		if(chipnum == 43362){
			val = WL_AUTH_OPEN_SHARED_BCM43362;
		}
#endif
//CY-
...
//Nate+ broadcom's patch for solving wifi disconnect when suspended
	...
	const wl_event_msg_t *e, void *data)
{
	...
	if (status == WLC_E_STATUS_PARTIAL) {
		...
		if ((wl) && (wl->p2p_net) && (wl->scan_request) && (wl->scan_request->wdev == wl->p2p_net) /*&&
			(wl->scan_request->wdev->netdev) && ( wl->scan_request->wdev->netdev->name) &&
			p2p_on(wl) && wl->p2p_supported && !strncmp(wl->scan_request->wdev->netdev->name,"p2p-",4)*/) {
#else
