...
//Nate+ broadcom's patch for GO performance
	if (((unsigned long)(skb->data)) & 0x1) {
		unsigned char *data = skb->data;
		uint32 length = skb->len;
		PKTPUSH(dhdp->pub.osh, skb, 1);
		memmove(skb->data, data, length);
		PKTSETLEN(dhdp->pub.osh, skb, length);
	}
//Nate-
...
//Nate+ broadcom patch for solving miracast performance problem
        if (ifidx != 0) {
...
//CY+ send HANG when softap FW no response
	dhd_pub_t *pub = &dhd->pub;
	char buf[WLC_IOCTL_SMLEN];
	int ret = 0;
	static int wlup_fail_count=0;
//CY-
...
//CY+ send HANG when softap FW no response
			if (pub->op_mode & DHD_FLAG_HOSTAP_MODE) {
				memset(buf, 0, sizeof(buf));
				bcm_mkiovar("ver", (char *)&buf, 4, buf, sizeof(buf));
				if ((ret  = dhd_wl_ioctl_cmd(dhd, WLC_GET_VAR, buf, sizeof(buf), FALSE, 0)) < 0) {
					wlup_fail_count++;
					printf("[SoftAP FW Hang] wl command failed count %d\n", wlup_fail_count);
					if(wlup_fail_count >= 5){
						net_os_send_hang_message(dhd_idx2net(pub, 0/*primary interface*/));
						wlup_fail_count=0;
					}
				} else
					wlup_fail_count=0;
...
//CY+ increase beacon loss timeout to 10s
//	uint bcn_timeout = 4;
	uint bcn_timeout =10;
//CY-
	uint retry_max = 3;
